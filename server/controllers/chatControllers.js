...
const accessChat = async (req,res,next) => {
    ...
    try{
        ...
        if(existingChatRoom.length>0){
            res.json({success:true,message:"chat successfuly fetched",chat: existingChatRoom});
        }else{
            ...
            try{
                ...
                try{
                    ...
                    console.log("error occured while fetching new chat with populated field ",err);
                    next(err);
                }
            }
            catch(err){
...
const getChats = async (req,res,next) =>{
    try{
        let chats = await ChatModel.find({users:{$elemMatch : {$eq: req.user._id}}}).populate("users","-password");
        res.json({
            success:true,
